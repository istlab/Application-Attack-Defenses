...
with the following metrics~\cite{GFDLS06,A00}:
\begin{itemize}
...
For example, a simple announcement that an attack defense mechanism
...
Sensitivity, specificity, {\sc ppv}, and {\sc npv}, are defined precisely by way
of the $2\times 2$ Table~\ref{tab:attacks-outcomes}. In the cells of
the table we distinguish:
\begin{itemize}
\item True Positive ({\sc tp}), a real attack that raises an alarm.
\item True Negative ({\sc tn}), an event that is not an attack and that does
  not raise an alarm.
\item False Positive ({\sc fp}), an event that although it is not an attack
  raises an alarm.
\item False Negative ({\sc fn}), an event that although is as attack does
  not raise an alarm.
\end{itemize}
...
\begin{equation}
\textrm{Sensitivity} = \frac{\textrm{{\sc tp}}}{\textrm{{\sc tp}} + \textrm{{\sc tn}}}
\end{equation}

\begin{equation}
\textrm{Specificity} = \frac{\textrm{{\sc tp}}}{\textrm{{\sc tn}} + \textrm{{\sc fp}}}
\end{equation}

\begin{equation}
\textrm{{\sc ppv}} = \frac{\textrm{{\sc tp}}}{\textrm{{\sc tp}} + \textrm{{\sc fp}}}
\end{equation}

\begin{equation}
\textrm{{\sc npv}} = \frac{\textrm{{\sc tn}}}{\textrm{{\sc tn}} + \textrm{{\sc fn}}}
\end{equation}
...
The calculation of {\sc ppv} and {\sc npv} requires that we evaluate the attack
...
operations, {\sc ppv} is what would determine our actions. Administrators
will limited resources need to allocate their attention judiciously.
They may need to prioritize responses to emergency warnings; the {\sc ppv}
