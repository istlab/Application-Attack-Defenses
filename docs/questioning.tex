...
(via social engineering again).
However, this technique does not involve code injection
and it is not described in Figure~\ref{fig:attacks}.
The {\sc csrf} attack illustrated here could
involve an attacker that manages to inject
{\sc html} code that contains a hidden request
in the way we described in Section~\ref{sec:attacks}.
This code will be stored in the database ({\sc csrf} 5)
and then it will re-enter the application if
a user visits a page ({\sc csrf} 6). When the
...
defenses that protect web applications against code injection attacks.
...
applications from code injection attacks.
...
the {\sc html} response that reaches the browser to
...
in the way we described in Section~\ref{sec:attacks}.
\noindent WebJail~\cite{VDDPJ11}, and {\sc soma}~\cite{OWVS08}
...
j{\sc csrf}~\cite{PS11} has a similar
...
{\sc sql}rand~\cite{BK04} implements {\sc isr} to detect {\sc
...
(Point {\sc ub}). Variations of this approach include
...
{\sc didafit}~\cite{LLW02} detects {\sc sql} injection (Point
...
following providers: the Common Vulnerabilities and Exposures ({\sc
  cve}) database~\cite{cve}, the Bugtraq security
...
application attacks based on code injection
