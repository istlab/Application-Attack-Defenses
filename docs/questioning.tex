...
\item We find that researchers often test the diagnostic performance
...
% In the last 15 years there are numerous mechanisms that have
% been developed to detect one or more of the attacks mentioned
% above. However, the majority of them are not used in practice.
...
from a real production environment. At the same time, this is what
limits their usefulness. When we obtain a test result in a production
environment, be it negative or positive, we really want to know how
much we should be worried, or relaxed. In other words, if an attack is
detected in a production environment, how much should we be worried?
If no attack is detected in a production environment, how relaxed
should we be that no attack has indeed taken place?
...
%% PL This paragraph repeats the material of the previous one, doesn't
%% it?
...
%% PL All of them more or less target web users, correct?
...
%% PL What kind of policies are these? Syntax-based, pattern matching?
...
%% PL In which way is this a kind of policy enforcement?
...
%% PL better to avoid characterisations and mention only the facts
%% (that it leads to many false positives, as shown by ...
...
%% PL You mean a lot of false alarms?
...
\egroup
\noindent
%% PL I do not understand what is specific to training here; most
%% classic SQL injection attacks work in this way.
...
attack~\cite{APKLM10}.
%% PL Like the return-to-lib attack?
...
% Maybe we should comment on the fact that these are
% frameworks coming from the etiological category
% and detect the attacks ``by design".
...
Table~\ref{tab:comp} shows for every mechanism its
corresponding overhead. In addition, it indicates
if the overhead is applied to the server-side ({\sc s}),
or the client-side ({\sc c}).
Our first observation, is that there is a differential
in the way in which the computational overhead of the
mechanisms is measured. In many cases, 16 out of
34 (47.05\%), the overhead is provided as a percentage
while in other cases, 7 our of 34 (20.5\%),
the authors provide the time (in terms of ms) that
their mechanisms add to the normal execution time of
the protecting entity. Surprisingly, in 10 cases (29,4\%),
the overhead was either not available ({\sc na}) or
not quantified ({\sc nq}). In the case of {\sc php aspis},
its authors indicate that with their mechanism,
...
19 frameworks (57,8\%) detect JavaScript-driven
...
out of 34 (17.6\%) of the authors provided a homepage for
...
out of 34 (8.8\%) authors have currently
...
We believe that our research may facilitate researchers
