...
then an exception is raised (locus {\sc z}). However, this is a simplistic
approach that may lead to many false positives.
{\sc csse}~\cite{PB05} and the system by {\it Xu et al.}~\cite{XBS06}
apply further checks when they identify that tainted data
have reached a sink. In particular, {\it Xu et al.}~\cite{XBS06}
...
regular expressions. {\sc csse} associates
...
(locus {\sc z}). {\it Xu et al.}~\cite{XBS06}
have proposed a 
In the case of {\sc dsl}-driven injection attacks,
the above implementations can be efficient. However, this
does not apply in the case of JavaScript injection.
Currently, with the wide use of JavaScript these mechanisms
would produce numerous false alarms because instead
of checking for tainted variables ``in" scripts,
they look inside {\sc html} responses ``for" scripts.
...
\scalebox{0.99}{
  ...
  &   {\it CsFire}~\cite{DDHPJ10} & & & {\bf ?} & {\sc csrf} \\
