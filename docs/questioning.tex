...
is because they are based on similar attack vectors as we described in
the previous section. We base the analysis of the existing defenses on
this model. Figure~\ref{fig:attacks} also illustrates in which step
the defenses detect corresponding attacks ({\sc todo}).

An attack starts with the malicious user sending an request either to
the server or to a web user. In the first case the attacker could
upload data containing a specially crafted script (for example, in a
blog comment) or create a request with additional {\sc sql} code.
