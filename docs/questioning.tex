...
\title{SoK: Defenses Against Web Application Attacks: From Research
...
At the same time,
attackers find new ways~\cite{HNSHS12,DKH14} to bypass defense
mechanisms using a variety of techniques, despite the numerous
countermeasures that are being introduced. As an example, already by 2006,
...
types of attacks. Then, we categorize a selection of these protection
mechanisms. In our selection we include approaches that
have been proposed in publications cited more than
...
Finally, we analyze each
mechanism across the following dimensions:
\begin{itemize}
...
Section~\ref{sec:attacks} presents our proposed model
of web application attacks based on source code injection.
...
{\sc sql} injection attack~\cite{CERT02,MS09,HVO06,SW06}. By using
similar techniques malicious users can perform other exploits based on
{\sc dsl}s like {\sc xp}ath~\cite{SW06,CDL07,MKS09}, {\sc
  xml}~\cite{MSM13} and {\sc json}~\cite{SMS13}. With this kind of
attacks, a malicious user can view sensitive information, destroy or
modify protected data, or even crash the entire application. {\sc
  html} is another {\sc dsl} that can be used for malicious purposes
