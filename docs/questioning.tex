...
An issue regarding this framework (as well as {\sc beep}
and {\sc dsi}) is that it cannot define policies that
restrict the behavior of third-party scripts introduced
by script inclusion and {\tt iframe} tags.
For instance, consider a web page (foo.net)
...
\noindent
The following frameworks can actually detect such
an attack (Point {\sc ub}).
...
{\it WebJail}~\cite{VDDPJ11}, is a framework similar to
ConScript. WebJail, contains the functionality of
...
Finally, {\sc soma}~\cite{OWVS08} requires site administrators
