...
\maketitle
\begin{abstract}
Web application attacks, can have different forms depending on the
execution context of the application and the location
of the programming flaw that leads to the attack.
Some of the most dangerous web application attacks,
like Cross-Site Scripting and {\sc sql} injection,
involve the exploitation of the following weakness:
programmers tend to develop applications that may
accept and process data of uncertain origin without
appropriate validation and / or filtering.
Such attacks have been constantly topping the lists
of the various security bulletin providers despite
the numerous countermeasures that have been proposed
over the past 15 years.

In this paper we first systematize the existing knowledge
regarding the various defense mechanisms by setting up
a model for application attacks that are based on
executable source code injection vulnerabilities. This model
highlights the key weakness behind such attacks
and helps us observe defenses from a common
perspective.

We then categorize and analyze the various
defense mechanisms that have been developed.
Our analysis is based on dimensions such as accuracy,
performance, deployment and security.
Especially notable for a key category is accuracy,
as our findings show that there are many
defense mechanisms that are tested in a poor manner.

By comparing the existing defenses and identifying their
common characteristics, we help: a) researchers
and practitioners to choose or modify a mechanism
according to their needs, and b) designers of
...
broken authentication and session management, and other issues.
...
Cross-Site Request Forgery ({\sc csrf}) attacks~\cite{LZRL09}
and others. Attacks like these top the vulnerability
lists of numerous bulletin providers like
the Open Web Application Security
Project\footnote{\url{https://www.owasp.org/index.php/Category:OWASP_Top_Ten_Project}}
({\sc owasp}) and {\sc mitre}\footnote{\url{http://cwe.mitre.org/top25/}}
for several years. Consider the case of {\sc owasp}'s Top
Ten project whose main goal is to raise awareness about
web application security by identifying some of the most
critical risks facing organizations which is referenced by
numerous researchers. In its three consecutive Top Ten lists
(2007, 2010, 2013), different executable source code-driven
injection attacks dominate the top five positions.
This indicates that apart from the fact that malicious
users find new ways to bypass defense
...
The first involves binary code and the second executable source code.
...
In this paper, we focus on the defenses that protect
applications against application attacks based on
the executable source code injection category.
...
is called an {\sc sql} injection attack~\cite{CERT02,MS09,HVO06,SW06}.
By using similar techniques malicious users
can perform other exploits based on {\sc dsl}s,
like {\sc xp}ath~\cite{SW06,CDL07,MKS09} and {\sc xml}~\cite{MSM13}.
With this kind of attacks, a malicious user can view sensitive
information, destroy or modify protected data, or even crash the entire
application.

A recent class of {\sc cia}s involve
dynamic languages like Python, Perl, JavaScript, and {\sc php}~\cite{SFVM09,EWKK09,SMS13}.
JavaScript injection attacks in particular, comprise a wide subset of dynamic
...
In this section we provide a step-by-step exploitation
model to understand the process of carrying out
application attacks based on executable source code injection
vulnerabilities. Figure~\ref{fig:attacks} presents the steps
of different application attacks. For the most part,
the steps are common for each attack. This is because
they are based on similar attack vectors as we described in
the previous section. We base the analysis of the
existing defenses on this model. Figure~\ref{fig:attacks} also
illustrates in which step the defenses detect corresponding
attacks ({\sc todo}).

An attack starts with the malicious user sending an
request either to the server or to a web user.
In the first case the attacker
could upload data containing a specially crafted script
(in a blog comment) or create a request with
additional {\sc sql} code.
...
code injection vulnerabilities ({\sc draft}).}
\end{figure*}

\section{Dimensions of Study}
...
\section{Web Application Attacks Defenses}
\label{sec:defs}

Figure~\ref{fig:defenses} presents a taxonomy of the
approaches that counter web application attacks based on
code injection vulnerabilities.
We can identify two main approaches, namely:
{\it protection} and {\it detection}.
...
Here we analyze the three different protection approaches.
...
two, has been applied to counter
...
consider a {\sc xor} function that encodes all JavaScript source of a web
page on the server-side and then, on the client-side, the web browser decodes the
source by applying the same function again.
...
intrusion detection framework~\cite{Den87}.
In particular, a training mechanism,
registers all valid legitimate code statements
during a training phase, most of the time in the form of signatures.
This can be done in various ways according to the implementation.
Then, only those will be accepted, approved or recognized during
production.
...
\begin{center}
\leavevmode
\includegraphics[scale=0.57]{defenses.pdf}
\end{center}
