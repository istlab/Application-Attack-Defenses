...
\begin{abstract}
  ...
  Accuracy is of particular importance, as our findings show that many defense
  ...
  modify defenses according to their needs, as well as designers of
  new approaches in developing more effective mechanisms.
...
and {\sc json}~\cite{SMS13}. With this kind of attacks,
...
\scalebox{0.93}{
  ...
  &   {\it Diglossia}~\cite{SMS13} & 4 & (9,{\sc nq},{\sc nq},{\sc nq})\_r & 13\% ({\sc s}) & {\sc sql}i and {\sc json}i \\
  \thickhline
    \end{tabular}}
    \begin{tablenotes}
  \begin{footnotesize}
...
An interesting observation is that mimicry attacks can be
used to bypass mechanisms coming from the hybrid category and
the policy enforcement and training subcategories.
...
In our research we tried to identify some of the reasons
why this is the case. Some of these reasons include
poor testing, high overhead, code unavailability,
deployment and security issues. However, our
systematization suggests that there are many
promising cases. For instance there are
mechanisms that have been extensively
tested in terms of accuracy ({\sc sqlc}heck, {\sc amnesia},
{\sc dsi}, libAnomaly), solve specific
problems in an efficient way (Blueprint and the system
by Stock et al.) or have a low computational overhead
({\sc dsi} and the system by Phung et al.).
There are also cases where researchers have made their
code available ({\sc beep} and {\sc sd}river) which
can be a push towards the development of better
mechanisms. This argument has been raised
by another {\sc s}o{\sc k} paper too~\cite{SPWS13}.
Finally, we believe that testing defenses in
a production setting could also propel their adoption.

We hope that the exploitation model, analysis
and observations that emerged from our research
can be a reference point for researcher who
wish to develop new countermeasures that
