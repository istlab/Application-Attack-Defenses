...
and then trick an end user to click on it via social engineering
(i.e., phishing). Alternatively, the attacker may be able to inject
directly the malicious code in the application. For example, consider
a web application that accepts and redisplays data of uncertain origin
without appropriate validation or filtering. An attacker could
...
request. Thus, in a non-persistent {\sc xss} attack the injected code
...
corresponding tuple that includes the step number for
every attack. {\sc p-xss} stands for a persistent {\sc xss}
attack and {\sc n-xss} stands for a non persistent {\sc xss}
attack. Note that {\sc xss} attacks can also involve
...
it available after the publication. The question mark (?),
...
the mechanisms detect corresponding attacks.
