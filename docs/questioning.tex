...
% dimitro@dimitro: change the above accordingly.
...
Point {\sc sb}~\cite{RDWDE07,JKK06a,JB07,NLC07,WPLKK09,JEP08},
Point {\sc eq}~\cite{BWS05,SW06,PS11,HCF05,XBS06,PB05,PMP11,MS09,HO05,SMS13},
...
acts as a proxy on the server side (Point {\sc SB}) to
...
{\sc SB}). Since all legitimate code has been ``masked", the injected
...
then an exception is raised (Point {\sc eq}).
...
tainted data have reached a sink (Point {\sc eq}). {\it Xu et
...
syntactic checks based on the aforementioned metadata (Point {\sc eq}).
...
if there are tainted data that contain scripts (Point {\sc sb}).
...
containing injected elements (Point {\sc eq}). {\sc
  amnesia}~\cite{HO05,HO06,HO05b} is a tool that also detects {\sc
  sql} injection attacks (Point {\sc eq}) by associating a query model
...
{\sc sb}) searches for injected scripts with no corresponding {\sc id}
...
compares it with the parsed tree of the ``shadow" (Point {\sc eq}). If
...
As we mentioned in Subsection~\ref{sec:deployment},
...
and mechanisms coming from the taint tracking
subcategory are such examples.
In the first two cases, programmers should modify every
...
Note also that in many cases, developers are required to
learn a new {\sc api}.

Finally, a disadvantage of mechanisms that involve training
({\sc sd}river, {\sc xss-guard}), is that when code statements
are altered, a new training phase is necessary. However,
with the increased adoption of the use of automated testing
...
frameworks (57,8\%) detect JavaScript-driven {\sc xss} attacks at the
...
has indicated that mechanisms that introduce an overhead
larger than 10\% do not tend to gain wide adoption in
production environments which in this case could be a
reason why some these tools have not been adopted.
