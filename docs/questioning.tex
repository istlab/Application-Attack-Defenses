...
Other factors matter as well. Reporting absolute measurements gives
little information on the real overhead, unless separate measurements
are given for the system under study with and without the proposed
mechanism. Percentage measurements are normally better. Outliers may
be important. It is always necessary to describe the experimental
setup.
...
these mechanisms is that they do
not examine the script's location inside the web
document. Based on this fact, attackers
can perform mimicry attacks~\cite{WS02}.
Specifically, they can execute legitimate scripts,
but not as intended by the original design of
the developers. This is extensively described
in reference~\cite{APKLM10}. In addition,
the authors of~\cite{APKLM10} describe
another recent variation of JavaScript
injection attacks that can be used to bypass the
above mechanisms, known as return-to-JavaScript
attacks.
% Such attacks share
...
would fail to detect mimicry attacks 
where legitimate scripts can be executed by
attackers, but not in the way intended by
the the developers~\cite{APKLM10}.
...
the following attack: Consider a banner rotator
...
the only framework that detects {\sc json} injection attacks. When an
