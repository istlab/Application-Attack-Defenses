...
\scalebox{0.99}{
	...
	&  	{\sc csse}~\cite{PB05} & 312 & \xmark,\xmark & 2--10\% & {\sc sql} injection \\
	&  	{\it Haldar et al.}~\cite{HCF05} & 177 & {\bf ?},{\bf ?} & \xmark & {\sc sql} injection \\ 
	&  	{\it SecuriFly}~\cite{MLL05} & 31 & \xmark,\tick & 9--125\% & {\sc sql} injection, {\sc xss} \\ 
	&  	{\it Xu et al.}~\cite{XBS06} & 297 & \tick,\xmark & 76\% & {\sc sql} injection, {\sc xss} \\ 
    &  	{\it {\sc wasc}}~\cite{NLC07} & 31 & \xmark,\xmark & 30\% & {\sc sql} injection, {\sc xss} \\
	&  	{\it Vogt et al.}~\cite{VFJKKV07} & 322 & \xmark,{\bf ?} & {\bf ?} & {\sc xss} \\
	&   {\it Noxes}~\cite{KKVJ06,KJKV09} & 268,40 & {\bf ?},{\bf ?},{\bf ?},{\bf ?} & {\bf ?} & {\sc xss} \\
	&  	{\it {\sc php} Aspis}~\cite{PMP11} & 12 & \xmark,\xmark & 2.2$\times$ & {\sc sql} and {\sc php} injection, {\sc xss} \\
	& 	{\it Stock et al.}~\cite{SLMS14} & 0 & 0.09\%,\xmark & 7--17\% & {\sc xss} \\
	\hline
	\hline      
	\multirow{3}{*}{{\sc isr}}
	&   {\it {\sc sql}rand}~\cite{BK04} & 286 & {\bf ?},{\bf ?} & 6.5{\it ms} & {\sc sql} injection \\ 
...
\scalebox{0.99}{
    ...
    \bf{Approach}
	& \bf{Mechanism}
    & \multicolumn{3}{|c|}{Availability} \\
	&& \bf{Source Code}
	& \bf{Executable}\tnote{4}
	& \bf{Testbed} \\
    \hline
	\multirow{9}{*}{Runtime Tainting}
	&  	{\sc csse}~\cite{PB05} & \xmark,\xmark & 2--10\% & - \\
	&  	{\it Haldar et al.}~\cite{HCF05}  & {\bf ?},{\bf ?} & \xmark & - \\ 
	&  	{\it SecuriFly}~\cite{MLL05}  & \xmark,\tick & 9--125\% & - \\ 
	&  	{\it Xu et al.}~\cite{XBS06}  & \tick,\xmark & 76\% & - \\ 
    &  	{\it {\sc wasc}}~\cite{NLC07} & \xmark,\xmark & 30\% & - \\
	&  	{\it Vogt et al.}~\cite{VFJKKV07}  & \xmark,{\bf ?} & {\bf ?} & - \\
	&   {\it Noxes}~\cite{KKVJ06,KJKV09}  & {\bf ?},{\bf ?},{\bf ?},{\bf ?} & {\bf ?} & - \\
	&  	{\it {\sc php} Aspis}~\cite{PMP11} & \xmark,\xmark & 2.2$\times$ & - \\
	& 	{\it Stock et al.}~\cite{SLMS14} & 0.09\%,\xmark & 7--17\% & - \\
	\hline
	\hline      
	\multirow{3}{*}{{\sc isr}}
	&   {\it {\sc sql}rand}~\cite{BK04}  & {\bf ?},{\bf ?} & 6.5{\it ms} & - \\ 
	&   {\it Noncespaces}~\cite{GC09}  & \tick,\tick &  10.3\% & - \\ 
    &   {\it x{\sc js}}~\cite{APKLM10} & \tick,\tick & 1.6--40{\it ms} & - \\
	\hline
	\hline    
	\multirow{13}{*}{Policy Enforcement}
	&   {\it {\sc dsi}}~\cite{NSS06}  & \xmark,\xmark & 1.85\% & - \\ 
	&   {\it BrowserShield}~\cite{RDWDE07}  & \tick,\tick & 8\% & - \\ 
	&   {\it CoreScript}~\cite{YCIS07} & {\bf ?},{\bf ?} &  {\bf ?} & - \\ 
	&   {\it {\sc met}}~\cite{ELX07} & {\bf ?},{\bf ?} &  {\bf ?} & - \\ 
    &   {\it {\sc beep}}~\cite{TNH07} & \tick,\tick & 14.4\% & - \\
    &   {\it {\sc soma}}~\cite{OWVS08}  & 5,0,0,0 & 5.58\% & - \\
