...
\emph{Availability}. The other four criteria correspond to \emph{Computational
...
typically introduce some amount of extra computation on a existing applications.
...
\emph{Instruction Set Randomization (ISR)}.
...
Two mechanisms that implement this approach for protection against {\sc SQL}
injection attacks,
...
is currently supported by many browsers to detect
...
\scalebox{0.93}{
  ...
  \begin{footnotesize}
       $^\textsuperscript{1}$The check mark (\tick) indicates that the publication
...
we considered in this work along the
...
In these cases, all the four elements of the quadruples
...
mechanisms were tested on real-world applications known to be
...
mentioned in the paper. We see that only 6 out of 35 (17.1\%) of the
...
frameworks (58,3\%) detect {\sc xss} attacks on the
...
with supported frameworks like Mozilla's {\sc csp}
and Google Caja~\cite{M06,caja}
