...
can take advantage of this and inject JavaScript code
into the page to steal the user's cookie for this particular
page and send it via an image request to a web site
that he or she controls ({\tt malicious.com}).
This could be achieved by including the following
code fragment to the malicious web site:

\lstset{language=VBScript, basicstyle=\footnotesize\ttfamily,}
\begin{lstlisting}
<iframe style="position:absolute;top:-9999px"
    src="http://foo.net/vulnerable.html?query=
...
\noindent
By using the {\tt iframe} tag, the web user will not
realise the redirection. 
This problem is actually solved by the architectures
