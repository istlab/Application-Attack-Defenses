...
The behavior-based category incorporates a variety of schemes that
inspect the behavior of applications and detect attacks based on
specific criteria~\cite{D76,A00}, that is, on specific behavioral
patterns. Hybrid mechanisms borrow characteristics from both
...
%% PL: Do we need this paragraph here? Before we say that we tread
%% availability in the upcoming ``Analysis'' section.
...
There exist three basic design-based approaches used to protect web
...
metadata, it takes advantage of specific features provided by {\sc
  php}.
%% PL Like what?
...
A recent study showed that there are ways to circumvent the majority
of the above schemes~\cite{NBR14}. Furthermore, most of them are not
...
site from which it came from. This though, may lead to false alarms.
...
$\Omega$) attacks by recording all database transactions. Subsequent
...
circumvented. For example, consider an application that will send the
...
the same JavaScript keywords like the above.
%% You mean like the rotator?
...
tracking approaches~\cite{NBR14}.
...
\scalebox{0.99}{
  ...
  &   {\it NoForge}~\cite{JKK06a} & 35 & (7,{\sc nq},{\sc nq},0)\_r & {\sc na} & {\sc csrf} \\
...
% the design-based category.

% Comment 2: Note though that implementations
% of design-based mechanisms can actually
% be bypassed.
