...
We then categorize and analyze a set of \hlc[yellow]{41} previously proposed defenses 
...
can follow the path of either a persistent {\sc xss}
...
An {\sc xfs} attack can be captured by our model
because the steps needed are pretty similar to the steps of
...
\begin{table*}[t]
  \renewcommand{\baselinestretch}{1.10}
    \caption{\hlc[yellow]{Summary of mechanisms developed to counter application
...
\hlc[yellow]{Information Flow Control ({\sc ifc})
...
{\sc jsf}low}~\cite{HBBS14}\hlc[yellow]{, {\sc cowl}}~\cite{SYMRHKM14}
\hlc[yellow]{and the framework by Bauer et al.}~\cite{BCJPST15}
\hlc[yellow]{allow developers to express information flow
policies be extending the type system of JavaScript.
Then,
the policies are enforced by the JavaScript interpreter
through dynamic checks.}
\hlc[yellow]{{\sc ifc} frameworks do not
...
(Servlet Information Flow)}~\cite{CVMA07}\hlc[yellow]{,
% Ur/Web}~\cite{C10b}\hlc[yellow]{,
Hails}~\cite{GLSTMMR12}
\hlc[yellow]{, and Aeolus}~\cite{CPSPBCCSL12}.
...
We group our key observations on the \hlc[yellow]{41} publications that
...
the approaches, \hlc[yellow]{20} out of \hlc[yellow]{41 (48.7\%)}, the overhead is provided as a
percentage, while in other cases, \hlc[yellow]{8} our of \hlc[yellow]{41 (19.5\%)}, the authors
...
time measurements to percentage overheads. In 10 cases (\hlc[yellow]{24.3\%}), the
overhead was either not available ({\sc na})
or not quantified ({\sc nq}).
\hlc[yellow]{For \textsc{php} Aspis}~\cite{PMP11}
\hlc[yellow]{and {\sc jsf}low}~\cite{HBBS14}
\hlc[yellow]{, the authors indicate that with
