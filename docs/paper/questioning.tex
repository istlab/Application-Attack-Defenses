...
\lstset{aboveskip=3.2pt,belowskip=3.1pt}
...
did not consider mechanisms that
have not been presented in research papers.
...
\begin{center}
\leavevmode
\includegraphics[scale=0.52]{attack-tree.png}
\end{center}
\vspace{-6mm}
...
\begin{center}
\leavevmode
\includegraphics[scale=0.46]{attacks-steps.png}
\end{center}
\vspace{-5.5mm}
...
and \textit{Document Object Model} ({\sc dom}) -
\textit{based} \textsc{xss}~\cite{SLMS14}.
...
{\sc dom}-based {\sc xss} attacks involve
...
A Cross Frame Scripting (\textsc{xfs})
...
where countermeasures are bypassed by savvy attacks, which are caught
by more sophisticated countermeasures, yet again bypassed by savvier
...
\begin{center}
\leavevmode
\includegraphics[scale=0.50]{defenses.png}
\end{center}
\vspace{-5.8mm}
...
\begin{table*}[t]
  \renewcommand{\baselinestretch}{1.10}
    \caption{Summary of mechanisms developed to counter application
    attacks based on code injection. The different results does not necessarily
    % \newline
    % \hspace{\linewidth}
    indicate that one mechanism is more effective than the other.
    This is because most of them were evaluated under different assumptions
    and settings.}
    \label{tab:comp}
    \vspace{-4.6mm}
\centering
...
\scalebox{0.76}{
  ...
  &   {\sc sqlg}uard~\cite{BWS05} & 368 & ({\sc na},{\sc na},{\sc na},{\sc na})\_{\bf ?} & 3\% ({\sc s}) & {\sc sql}i \\
  &   {\sc sqlc}heck~\cite{SW06} & 547 & (36848,7648,0,0)\_r & 3ms per query ({\sc s}) & {\sc sql}i \\
  \hline
  \multirow{13}{*}{Policy Enforcement}
  &   {\sc dsi}~\cite{NSS06} & 188 & (5268,{\sc nq},{\sc nq},85)\_r & 1.85\% ({\sc c}) & {\sc xss} \\ 
  &   NoForge~\cite{JKK06a} & 166 & (7,{\sc nq},{\sc nq},0)\_r & {\sc na} ({\sc s}) & {\sc csrf} \\
  &   Noxes~\cite{KJKV09} & 69 & (3,{\sc na},{\sc na},0)\_r & {\sc na} ({\sc c}) & {\sc xss} \\
  % &   {\it {\sc met}}~\cite{ELX07} & 58 & ({\sc na},{\sc na},{\sc na},{\sc na})\_{\bf ?} & {\sc na} & {\sc xss} \\ 
  &   {\sc beep}~\cite{TNH07} & 362 & (61,{\sc na},{\sc na},0)\_r & 14.4\% ({\sc c}) & {\sc xss} \\
  &   BrowserShield~\cite{RDWDE07} & 273 & (19,{\sc nq},0,0,)\_r & 8\% ({\sc s}) & {\sc xss} \\ 
  &   CoreScript~\cite{YCIS07} & 212 & ({\sc nq},{\sc nq},{\sc nq},{\sc na})\_s & {\sc nq} ({\sc c}) & {\sc xss} \\
  &   {\it {\sc soma}}~\cite{OWVS08} & 65 & (5,{\sc na},{\sc na},0)\_s & 5.58\% ({\sc c}) & {\sc xss}, {\sc csrf} \\
  % &   {\it Barth et al.}~\cite{BJM08} & 271 & & {\bf ?} & {\sc csrf} \\
  &   Phung et al.~\cite{PSC09} & 124 & (37,{\sc na},{\sc na},4)\_r & 5.37\% ({\sc c}) & {\sc xss} \\
  &   ConScript~\cite{ML10} & 152 & ({\sc na},{\sc na},{\sc na},{\sc na})\_{\bf ?} & 7\% ({\sc c}) & {\sc xss} \\
  &   CsFire~\cite{DDHPJ10} & 52 & (419582,1141807,0,3)\_r\tnote{5} & {\sc na} ({\sc c}) & {\sc csrf} \\
  &   {\sc csp}~\cite{SSM10} & 143 & ({\sc na},{\sc na},{\sc na},{\sc na})\_{\bf ?} & {\sc nq} ({\sc c}) & {\sc xss}, {\sc csrf} \\
  &   j{\sc csrf}~\cite{PS11} & 4 & (2,{\sc na},{\sc na},0)\_r & 2ms ({\sc s}) & {\sc csrf} \\
  &   WebJail~\cite{VDDPJ11} & 53 & (2,{\sc na},{\sc na},1)\_{\bf ?} & $\sim$6.89ms ({\sc c}) & {\sc xss} \\
  ...
  \hline
  \multirow{4}{*}{{\sc isr}}
  &   {\sc sql}rand~\cite{BK04} & 428 & (3,{\sc na},{\sc na},0)\_a & $\le$6.5{\it ms} ({\sc s}) & {\sc sql}i \\
  &   {\sc sm}ask~\cite{JB07} & 31 & (5,{\sc nq},{\sc nq},{\sc nq})\_r  & {\sc na} ({\sc s}) & {\sc sql}i, {\sc xss} \\
  &   Noncespaces~\cite{GC09} & 146 & (6,{\sc na},{\sc na},0)\_r &  2\% ({\sc s}) & {\sc xss} \\ 
  &   x{\sc js}~\cite{APKLM10} & 29 & (1380,{\sc na},{\sc na},1)\_r & 1.6--40ms ({\sc c}) & {\sc xss} \\
  \thickhline
  \thickhline
  \multirow{7}{*}{Taint Tracking}
  &   Haldar et al.~\cite{HCF05} & 234 & (2,{\sc na},{\sc na},0)\_s & {\sc nq} ({\sc s}) & {\sc sql}i, {\sc xss} \\ 
  &   {\sc csse}~\cite{PB05} & 387 & (7,{\sc nq},{\sc nq},{\sc nq})\_r & 2--10\% ({\sc s}) & {\sc sql}i, {\sc xp}athi, {\sc xss} \\
  % &   {\it SecuriFly}~\cite{MLL05} & 31 & \xmark,\tick & 9--125\% & {\sc sql} injection, {\sc xss} \\ 
  &   Xu et al.~\cite{XBS06} & 368 & (9,{\sc nq},0,{\sc nq})\_r & average 76\% ({\sc s}) & {\sc sql}i, {\sc xss} \\ 
  &   {\sc wasc}~\cite{NLC07} & 37 & ({\sc nq},{\sc nq},{\sc nq},{\sc nq})\_r & up to 30\% ({\sc s}) & {\sc sql}i, {\sc xss} \\
  &   Vogt et al.~\cite{VFJKKV07} & 490 & ({\sc nq},{\sc nq},{\sc nq},{\sc na})\_r & {\sc nq} ({\sc c}) & {\sc xss} \\
  &   {\sc php} Aspis~\cite{PMP11} & 26 & (12,{\sc nq},{\sc nq},2)\_r & 2.2$\times$ ({\sc s}) & {\sc sql}i and {\sc php}i, {\sc xss} \\
  &   Stock et al.~\cite{SLMS14} & 23 & (1169,{\sc na},{\sc na},0)\_r & 7--17\% ({\sc c}) & {\sc dom}-based {\sc xss} \\
  \hline 
  \multirow{6}{*}{Training}
  &   {\sc didafit}~\cite{LLW02} & 208 & ({\sc na},{\sc na},{\sc na},{\sc na})\_{\bf ?} & {\sc na} ({\sc s}) & {\sc sql}i \\
  &   {\sc amnesia}~\cite{HO05b} & 551 & (1470,{\sc nq},0,0)\_a & {\sc nq} ({\sc s}) & {\sc sql}i \\ 
  &   libAnomaly~\cite{VMV05} & 310 & (9,15987,60,0)\_r & 0.20--1ms per query ({\sc s}) & {\sc sql}i \\
  &   {\sc xssds}~\cite{JEP08} & 96 & ({\sc nq},{\sc nq},{\sc nq},0)\_r & {\sc nq} ({\sc s}) & {\sc xss} \\
  &   {\sc swap}~\cite{WPLKK09} & 83 & ({\sc nq},{\sc nq},{\sc nq},{\sc nq})\_r & up to 261ms ({\sc s}) & {\sc xss} \\ 
  &   {\sc sd}river~\cite{MS09,MKLS11} & 36 & (241,{\sc nq},0,0)\_a & 39\% ({\sc s}) & {\sc sql}i and {\sc xp}athi \\
  % &   {\it Laranjeiro et al.}~\cite{LVM09,ALVM09,LVM10} & 9,40,1 & \xmark,\xmark  & \xmark & {\sc sql} and {\sc xp}ath injection \\
  \thickhline
  \thickhline
  \multirow{9}{*}{Hybrid}
  &   {\sc xss-guard}~\cite{BV08} & 153 & (8,{\sc nq},{\sc nq},{\sc nq})\_r & 5--24\% ({\sc c}) & {\sc xss} \\
  &   Blueprint~\cite{LV09} & 187 & (94,{\sc na},{\sc na},0)\_r & 13.6\% ({\sc c}) & {\sc xss} \\
  &   Diglossia~\cite{SMS13} & 23 & (25,{\sc nq},{\sc nq},{\sc nq})\_r & 13\% ({\sc s}) & {\sc sql}i and {\sc json}i \\
  &   {\sc jsf}low~\cite{HBBS14} & 72 & ({\sc nq},{\sc nq},{\sc nq},{\sc nq})\_r
  &   2$\times$ ({\sc c}) & {\sc xss} \\
  &   {\sc cowl}~\cite{SYMRHKM14} & 41 & ({\sc nq},{\sc nq},{\sc nq},{\sc nq})\_r
  &   16\% ({\sc c}) & {\sc xss} \\
  &   Bauer et al.~\cite{BCJPST15} & 10 & ({\sc na},{\sc na},{\sc na},{\sc na})\_{\bf ?}
  &   average 55\% ({\sc c}) & {\sc xss} \\
  &   {\sc sif}~\cite{CVMA07} & 155 & ({\sc na},{\sc na},{\sc na},{\sc na})\_{\bf ?}
  &   26\% ({\sc s}) & {\sc sql}i, {\sc xss} \\
  % &   \hlc[yellow]{Ur/Web}~\cite{C10b} & \hlc[yellow]{59} & \hlc[yellow]{({\sc nq},{\sc nq},{\sc nq},{\sc na})\_s}
  % & \hlc[yellow]{average 55\% ({\sc s})} & \hlc[yellow]{{\sc sql}i} \\
  &   Hails~\cite{GLSTMMR12} & 92 & ({\sc nq},{\sc nq},{\sc nq},{\sc nq})\_r
  &   28\% ({\sc s},{\sc c}) & {\sc sql}i, {\sc xss} \\
  &   Aeolus~\cite{CPSPBCCSL12} & 58 & ({\sc na},{\sc na},{\sc na},{\sc na})\_{\bf ?}
  &   323.5ms per request ({\sc s}) & {\sc sql}i, {\sc xss} \\
  \thickhline
    \end{tabular}}
    \vspace{-0.5mm}
    \begin{tablenotes}
...
These policies are communicated via the {\sc http} headers.
Then, the browser
...
In this way, {\sc xss} attacks that take advantage
...
Keeping track of such input may be impractical not only because of the
...
Five of them focus on the detection of {\sc xss}
...
Information Flow Control ({\sc ifc})
mechanisms combine positive taint tracking and
policy enforcement to prevent {\sc xss} attacks on the
client-side (Point {\sc ub}).
Representative implementations such as
{\sc jsf}low~\cite{HBBS14}, {\sc cowl}~\cite{SYMRHKM14}
and the framework by Bauer et al.~\cite{BCJPST15}
allow developers to express information flow
policies by extending the type system of JavaScript.
Then,
the policies are enforced by the JavaScript interpreter
through dynamic checks.
{\sc ifc} frameworks do not
focus explicitly on JavaScript and they
have a broader scope.
They can be used to build secure applications,
thus preventing different attacks.
Policies can be provided either as compile-time
program annotations,
or as run-time requirements defined by the user.
Such frameworks include {\sc sif}
(Servlet Information Flow)~\cite{CVMA07},
% Ur/Web}~\cite{C10b}\hlc[yellow]{,
...
We group our key observations on the 41 publications that
...
\begin{center}
\leavevmode
\includegraphics[scale=0.40]{barchart.png}
\end{center}
\vspace{-6mm}
...
\scalebox{0.69}{
  ...
  &   {\sc jsf}low~\cite{HBBS14} & \tick & \tick & {\sc na} \\
  &   {\sc cowl}~\cite{SYMRHKM14} & {\sc na} & {\sc na} & {\sc na} \\
  &   Bauer et al.~\cite{BCJPST15} & {\sc na} & {\sc na} & {\sc na} \\
  &   {\sc sif}~\cite{CVMA07} & {\bf AO} & {\sc na} & {\sc na} \\
  % &   \hlc[yellow]{Ur/Web}~\cite{C10b} & \hlc[yellow]{59} & \hlc[yellow]{({\sc nq},{\sc nq},{\sc nq},{\sc na})\_s}
  % & \hlc[yellow]{average 55\% ({\sc s})} & \hlc[yellow]{{\sc sql}i} \\
  &   Hails~\cite{GLSTMMR12} &  {\bf AO} & {\sc na} & {\bf AO} \\
  &   Aeolus~\cite{CPSPBCCSL12} &  \tick & \tick & {\sc na} \\
  \thickhline
...
only 4 out of 41 (9.7\%) publications have currently their testbeds
available.
...
the approaches, 20 out of 41 (48.7\%), the overhead is provided as a
percentage, while in other cases, 8 our of 41 (19.5\%), the authors
provide the latency (in ms) that their mechanisms add to the
normal execution time of the protected application.
Note that in some
cases, the times of normal executions are not provided
so it is not possible to convert absolute
time measurements to percentage overheads. In 10 cases (24.3\%), the
overhead was either not available ({\sc na})
or not quantified ({\sc nq}).
For \textsc{php} Aspis~\cite{PMP11}
and {\sc jsf}low~\cite{HBBS14}
the authors indicate that with
their mechanisms the execution time is doubled.
...
provide a simple security model
...
attacks, 3 out of 16 (18.7\%) do so at the level
of the {\sc rdbms} (Point {\sc db}).
The other 13 are based on interposing on {\sc api} calls related to output
...
A reasonable argument would be that many defenses
...
This work was supported by {\sc nsf} Grant No. {\sc cns}-13-18415.
